# -*- coding: utf-8 -*-
"""model5_2_not_change.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1JFqXll2ut3PZoePmm04176jr36RKkJZy
"""
...
for i in range(num_layers):
    X = data[layer_features[i]].dropna().reset_index(drop=True)
    layer_data_points[i] = len(X)

normalized_layer_sse_pca = {}
normalized_layer_sse_tsne = {}
for i in range(num_layers):
    normalized_layer_sse_pca[i] = layer_sse_pca[i] / layer_data_points[i]
    normalized_layer_sse_tsne[i] = layer_sse_tsne[i] / layer_data_points[i]

# 正規化SSEを表示
print("\nNormalized SSE for each layer:")
print("PCA:")
for layer, sse in normalized_layer_sse_pca.items():
    print(f"Layer {layer}: {sse}")
print("\nt-SNE:")
for layer, sse in normalized_layer_sse_tsne.items():
    print(f"Layer {layer}: {sse}")